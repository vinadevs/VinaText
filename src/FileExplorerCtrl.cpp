...
BEGIN_MESSAGE_MAP(CFileExplorerCtrl, FILETREECTRL_BASE_CLASS) //NOLINT(modernize-avoid-c-arrays)
	...
	ON_COMMAND(ID_TREEFILECTRL_OPEN_BY_EXPLORER_EXTRA, &CFileExplorerCtrl::OnCreateDocumentFromFile)
	ON_COMMAND(ID_TREEFILECTRL_OPEN_BY_AS_RAW_FORMAT, &CFileExplorerCtrl::OnOpenAsRawFormat)
	ON_COMMAND(ID_TREEFILECTRL_OPEN_ALL_BY_EXPLORER_EXTRA, &CFileExplorerCtrl::OnOpenAllFileByVinaText)
...
#pragma warning(suppress: 26496)
			...
			if (dwAttributes != INVALID_FILE_ATTRIBUTES)
			{
				...
				{ 
					...
					pPopup->DeleteMenu(ID_TREEFILECTRL_OPEN_ALL_BY_EXPLORER_EXTRA, MF_BYCOMMAND);
				...
				else // NOT FOR FOLDER...
				{
					...
					pPopup->DeleteMenu(ID_TREEFILECTRL_OPEN_BY_EXPLORER_EXTRA, MF_BYCOMMAND);	
...
void CFileExplorerCtrl::DoNavigateSearchFiles(CString strFolderPath)
{
	...
	else
	{
		...
		LOG_OUTPUT_MESSAGE_ACTIVE_PANE(strMessage, BasicColors::orangered);
	}
}
...
void CFileExplorerCtrl::OnCut()
{
	...
	if (hItem)
	{
		if (IsFolder(hItem))
		{
			LOG_OUTPUT_MESSAGE_ACTIVE_PANE(_T("> [From Explorer Window] Please use system menu for cutting folder [Shift + Right click]..."), BasicColors::orangered);
		}
		else
		{
...
void CFileExplorerCtrl::OnCopy()
{
	...
	if (hItem)
	{
		if (IsFolder(hItem))
		{
			LOG_OUTPUT_MESSAGE_ACTIVE_PANE(_T("> [From Explorer Window] Please use system menu for copying folder [Shift + Right click]..."), BasicColors::orangered);
		}
		else
		{
...
void CFileExplorerCtrl::OnPreviewFile(_In_ NMTREEVIEW* /*pNMTreeView*/, _In_ const CString& sPath)
{
	...
	if (!PathUtils::IsBinaryFile(sPath, FILE_BINNARY | FILE_OFFICE))
	{
		...
		if (llFileSize > AppSettingMgr.m_nFilePreviewSizeLimit)
		{
			...
			LOG_OUTPUT_MESSAGE_ACTIVE_PANE(AfxCStringFormat(_T("> [From Explorer Window] [Preview Waring] File size %s is %s, exceeded preview limitation. Please change limitation value in setting..."), sPath, strFileSize), BasicColors::orangered);
			return;
		}
