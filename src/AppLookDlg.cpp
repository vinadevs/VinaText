...
CAppLookDlg::CAppLookDlg(BOOL bStartup, CWnd* pParent /*=NULL*/) :
	CDialogEx(CAppLookDlg::IDD, pParent), m_bStartup(bStartup)
{
...
BEGIN_MESSAGE_MAP(CAppLookDlg, CDialogEx)
...
void CAppLookDlg::SetLook()
{
	...
	if (pMainFrame != NULL)
	{
		pMainFrame->OnChangeAppLook(m_bOneNoteTabs/* OneNote tabs */, m_bOneNoteTabs /* MDI tab colors*/, m_nAppLook != 0 /* VS.NET look */, m_bDockTabColors /* Dock tab colors*/, m_nAppLook == 1 || m_nAppLook == 4 /* VS 2005/2008 MDI tabs */, m_bActiveTabCloseButton);
...
void CAppLookDlg::OnAppLook()
{
	...
	m_wndRoundedTabs.EnableWindow(m_nAppLook == 1);
	m_wndStyle.EnableWindow(m_nAppLook == 3);
}
